---
name: flow-architect
description: 当用户请求生成流程初稿、评审流程架构工件、评审流程图或两者兼有时使用。本技能是 Flow Architect 技能族的总入口，根据输入类型选择流程初稿或流程评审路线。支持离线 HTML 会议包生成。
---

# Flow Architect 总入口

Flow Architect 技能族的顶层编排技能。检查输入工件，识别格式与类型，选择业务入口（流程初稿或流程评审），并将执行委派给对应的下游技能。本技能不直接执行领域评审。支持生成离线 HTML 会议包用于线下讨论。
