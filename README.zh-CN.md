...
`setup` 默认安装 core，PDF、DOCX、XLSX、PPTX 组件可按需选择安装；依赖写入用户缓存，不写入 Git 仓库或插件目录。
...
此命令是只读的，会展示所有可用技能、运行时状态、支持格式、示例和诊断信息，不修改任何文件。能看到能力表格和运行时状态，即说明安装成功。

## 入口 Skill

| Skill | 用途 |
|---|---|
| `flow-architect` | 默认入口；盘点输入并路由至流程初稿或对应评审路线 |
...
| `flow-architect-quickstart` | 自然语言正式入口；先确定性枚举候选公共方法，经用户确认后路由到对应的严格入口 |

默认入口 Skill（`flow-architect`）会盘点输入文件、识别制品族，并按意图路由：需要生成初稿时委派至 `flow-architect-draft-process`；评审时按制品族选择联合评审、仅架构评审或仅流程图评审。如果已经明确需要哪种评审，也可以直接调用三个流程 Skill。
...
评审技能为**只读**：检查现有制品、输出结构化 Finding，不修改、不创建、不修复任何用户文件。

创建技能（`flow-architect-draft-process`、`flow-architect-build-meeting-package`）**仅在**用户授权的独立运行目录中创建新制品，不修改原始输入。初稿生成全程确定性：抽取、分批、BPMN 生成、HTML 打包均为纯代码（零 LLM），仅逐批语义解释时调用 LLM。两条路径互补：初稿产出可评审的制品，评审技能评估这些制品。
...
| XLSX (.xlsx) | ✅ 完整支持 | 按实际 OOXML 内容动态分类，支持表格、原生 DrawingML、图片及混合输入；只有明确的 ID 关系才形成连接，缺失或有歧义时给出 warning 并降级，不按几何距离猜测连线 |
...
**L5 活动与 Task 关系：** 每个 L5 活动恰好对应一个主 Task，最多再带一个串行确认从 Task；网关、中间事件和结束事件不算 L5 活动。
...
准备阶段的结果按运行目录缓存。相同输入重新运行时，缓存批次直接复用（队列状态 `CACHED`），仅新增或发生变更的输入进入 `PENDING`。若缓存批次损坏（哈希不匹配、证据漂移），对应项回退为 `PENDING`，其他合法缓存项仍保持 `CACHED`。

## 流程初稿 — 焦点预检与诚实预算

- **焦点只读预检：** 落盘前先做候选发现预检（零文件系统变化）；检测到多个流程候选且未指定焦点时，只返回一个证据驱动问题、不创建运行目录；选定焦点后只处理焦点子集。
- **诚实预算：** dry-run 估计区分 `EXACT`（精确值，与实际 blocks/batches/tasks 一致）与 `HEURISTIC_RANGE`（启发式区间），不会把启发式值标成精确"预计批次"；dry-run 全程零写入。
- **失败自动恢复：** 语义片段 JSON 不可解析、Schema 校验失败或缺少 uncertainty 时，由确定性 orchestrator 记录原因并以 fresh worker 重试（最多 3 次）；运行报告记录每个任务的尝试次数、失败原因与最终片段哈希。
- **流程卡片真实性：** 流程卡片的 `owner`、`purpose` 取焦点流程的明确事实值；真正缺失时为 `null`，界面显示"待确认"但不回写为业务值，不使用硬编码占位值。
...
- **分批：** 证据按 ≤12,000 字符、≤12 个块（block）、≤1 个视觉资产（visual）拆批，每批附上下文预算报告。
- **预算门禁：** 三态预算（BUDGET_OK / BUDGET_ATTENTION / BUDGET_SPLIT_REQUIRED）。超过 token 上限 120% 的批次禁止启动 Worker。
- **BPMN 生成：** 从合并的语义片段生成 L5 BPMN 2.0 XML + DI。
- **HTML 打包：** 组装包含流程图、问题和元数据的离线会议包。

仅逐批的语义解释（产出 fragment）可能调用 LLM Worker。
...
图片和扫描 PDF 以视觉方式分析。仅依据图片输入得出的结构推断，置信度上限为 0.6，可能需要业务确认；插件不会从图片中产出虚假的精确 BPMN 元素级结论。
...
评审路线各阶段（如 `extract-architecture`、`review-l4`、`review-bpmn`、`review-consistency`）的结果写入 `stages/<stage-id>/result.json`；最终裁决 `review-verdict.json` 写入运行目录根，并生成汇总报告。流程初稿运行目录额外使用 `stages/semantic/`（批次队列与语义片段）和 `stages/merge/`（合并事实与合并报告），最终制品写入 `final/`：`process.bpmn`、`questions.json`、`clarification-agenda.md`、`process-draft.json` 和离线 HTML 会议包。

运行根目录由用户或目标项目决定，不会写入插件安装目录。
