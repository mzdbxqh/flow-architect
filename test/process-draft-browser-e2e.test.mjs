...
test('真实多源 r01 浏览器端到端：编辑、导出 r02、重新打开并验证', async t => {
  ...
  const renamedContract = await page.evaluate((taskId) => {
    const snapshot = window.__FLOW_ARCHITECT__.store.snapshot();
    const binding = snapshot.diagram.task_bindings.find(item => item.main_task_id === taskId);
    return {
      selectedType: window.__FLOW_ARCHITECT__.diagramController.selected?.type,
      binding,
      node: snapshot.diagram.nodes.find(item => item.node_id === taskId),
      activity: binding
        ? snapshot.activities.find(item => item.activity_id === binding.activity_id)
        : null,
    };
  }, firstTaskId);
  assert.equal(renamedContract.node?.name, '线下确认活动', '业务合同中的主 Task 名称应同步修改');
  assert.equal(renamedContract.activity?.name, '线下确认活动', '活动一览表中的活动名称应同步修改');
