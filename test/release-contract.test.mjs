...
test('v0.4.0 is consistent across package and every published manifest', () => {
  ...
  for (const file of [
    ...
    assert.equal(json(file).version, '0.4.0', `${file} version must be 0.4.0`);
  }

  assert.equal(json('.claude-plugin/marketplace.json').plugins[0].version, '0.4.0');

  // Root .claude-plugin/plugin.json is generated by build-adapters.mjs
  assert.equal(json('.claude-plugin/plugin.json').version, '0.4.0');
...
test('adapter generator is the version and capability source of truth', () => {
  ...
  assert.match(generator, /const PLUGIN_VERSION = '0\.4\.0'/);
