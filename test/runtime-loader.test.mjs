...
// ═══════════════════════════════════════════════════════════════════════════
// 行为 2: cache-only 模式下，损坏缓存即使存在本地包也返回 FLOW_ARCHITECT_RUNTIME_MISSING
// ═══════════════════════════════════════════════════════════════════════════

test('cache-only 模式下，损坏缓存即使存在本地包也返回 FLOW_ARCHITECT_RUNTIME_MISSING', async (t) => {
  ...
  assert.equal(result.status, 0, `应该抛出异常: ${result.stderr?.toString()}`);
  const output = JSON.parse(result.stdout.toString());
  assert.equal(output.threw, true, '应该抛出异常');
  assert.equal(output.code, 'FLOW_ARCHITECT_RUNTIME_MISSING', '应该返回 FLOW_ARCHITECT_RUNTIME_MISSING');
  assert.equal(output.component, 'core', '组件应该是 core');
  assert.equal(output.specifier, 'fast-xml-parser', 'specifier 应该是 fast-xml-parser');
});
...
test('cache-only 模式下，损坏缓存返回 FLOW_ARCHITECT_RUNTIME_MISSING', async (t) => {
  ...
  // 损坏缓存中的 core 包，使其状态变为非 READY
  const coreNmDir = path.join(cacheDir, 'runtimes', '2.0.0', 'core', 'node_modules', 'fast-xml-parser');
  if (fs.existsSync(coreNmDir)) {
    fs.writeFileSync(path.join(coreNmDir, 'index.js'), 'throw new Error("corrupted cache")');
  }
  ...
  assert.equal(result.status, 0, `进程应正常退出: ${result.stderr?.toString()}`);
  const err = JSON.parse(result.stdout.toString());
  assert.equal(err.code, 'FLOW_ARCHITECT_RUNTIME_MISSING', '应返回结构化能力错误');
  assert.equal(err.component, 'core', '组件应为 core');
  assert.equal(err.specifier, 'fast-xml-parser', 'specifier 应为 fast-xml-parser');
});
